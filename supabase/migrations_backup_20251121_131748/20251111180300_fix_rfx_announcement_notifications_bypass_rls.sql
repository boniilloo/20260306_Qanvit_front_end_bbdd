-- Fix create_notifications_on_rfx_announcement to properly bypass RLS
-- The function needs to read all rfx_company_invitations for the RFX, not just those visible to the current user
-- We'll use a subquery that explicitly checks the RFX ownership to ensure we can read the invitations
...
-- The function owner (usually postgres or service_role) should be able to read all data
...
'Creates in-app and email notifications to all companies related to an RFX when an announcement is posted (SECURITY DEFINER). Uses SECURITY DEFINER to bypass RLS and read all rfx_company_invitations for the RFX.';
