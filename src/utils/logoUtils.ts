    ...
    analyzeImageBrightness(logoUrl)
      .then(analysis => {
        ...
        if (analysis.averageBrightness > 0 && analysis.method !== 'skip-jpg') {
          ...
          reason = isWhite ? `Brillo alto: ${analysis.averageBrightness.toFixed(1)} > 150` : `Brillo bajo: ${analysis.averageBrightness.toFixed(1)} ≤ 150`;
          
          setNeedsDarkBg(isWhite);
        } else if (analysis.method === 'skip-jpg') {
          method = 'skipped';
          reason = 'Imagen JPG/JPEG: análisis desactivado';
          setNeedsDarkBg(false);
        } else {
          // Analysis failed
          method = 'error';
          reason = 'Análisis falló - CORS bloquea el análisis';
          setNeedsDarkBg(false);
        }
      ...
      .catch((error) => {
        ...
        setAnalysisData({
          averageBrightness: 0,
          whitePixelRatio: 0,
          reason: 'Error en análisis de imagen',
          method: 'error'
        });
        setIsAnalyzing(false);
      });
