...
-- Removed net schema and http_post function creation due to permission issues
-- These appear to already exist in the system
...
revoke delete on table "public"."test_products" from "anon";

revoke insert on table "public"."test_products" from "anon";

revoke references on table "public"."test_products" from "anon";

revoke select on table "public"."test_products" from "anon";

revoke trigger on table "public"."test_products" from "anon";

revoke truncate on table "public"."test_products" from "anon";

revoke update on table "public"."test_products" from "anon";

revoke delete on table "public"."test_products" from "authenticated";

revoke insert on table "public"."test_products" from "authenticated";

revoke references on table "public"."test_products" from "authenticated";

revoke select on table "public"."test_products" from "authenticated";

revoke trigger on table "public"."test_products" from "authenticated";

revoke truncate on table "public"."test_products" from "authenticated";

revoke update on table "public"."test_products" from "authenticated";

revoke delete on table "public"."test_products" from "service_role";

revoke insert on table "public"."test_products" from "service_role";

revoke references on table "public"."test_products" from "service_role";

revoke select on table "public"."test_products" from "service_role";

revoke trigger on table "public"."test_products" from "service_role";

revoke truncate on table "public"."test_products" from "service_role";

revoke update on table "public"."test_products" from "service_role";
...
alter table "public"."test_products" alter column "id" set default nextval('public.test_products_id_seq'::regclass);
...
CREATE TRIGGER trg_auth_users_email_confirmed AFTER UPDATE ON auth.users FOR EACH ROW WHEN (((new.email_confirmed_at IS NOT NULL) AND ((old.email_confirmed_at IS NULL) OR (old.email_confirmed_at <> new.email_confirmed_at)))) EXECUTE FUNCTION public._notify_on_email_confirmed();
  ...
  create policy "Allow authenticated users to delete chat documents"
  on "storage"."objects"
  as permissive
  for delete
  to public
using (((bucket_id = 'chat-documents'::text) AND (auth.role() = 'authenticated'::text)));



  create policy "Allow authenticated users to upload chat documents"
  on "storage"."objects"
  as permissive
  for insert
  to public
with check (((bucket_id = 'chat-documents'::text) AND (auth.role() = 'authenticated'::text)));



  create policy "Allow authenticated users to view chat documents"
  on "storage"."objects"
  as permissive
  for select
  to public
...
using (((bucket_id = 'chat-documents'::text) AND public.has_developer_access()));
  ...
  create policy "Anyone can view company logos and product images"
  on "storage"."objects"
  as permissive
  for select
  to public
...
using ((bucket_id = 'company-logos'::text));
  ...
  create policy "Anyone can view public conversation images"
  on "storage"."objects"
  as permissive
  for select
  to public
using ((bucket_id = 'public-conversation-images'::text));
...
with check (((bucket_id = 'company-logos'::text) AND (EXISTS ( SELECT 1
  ...
  for insert
  to public
with check (((bucket_id = 'product-documents'::text) AND (auth.uid() IS NOT NULL)));
  ...
  for insert
  to public
with check (((bucket_id = 'temp-pdfs'::text) AND (auth.role() = 'authenticated'::text)));
  ...
  for select
  to public
using (((bucket_id = 'product-documents'::text) AND (auth.uid() IS NOT NULL)));
...
using ((bucket_id = 'company-documents'::text));
  ...
  create policy "Give anon users access to JPG images in folder 4impie_0"
  on "storage"."objects"
  as permissive
  for select
  to public
using (((bucket_id = 'fq-logos'::text) AND (storage.extension(name) = 'jpg'::text) AND (lower((storage.foldername(name))[1]) = 'public'::text) AND (auth.role() = 'anon'::text)));



  create policy "Users can delete their own admin request documents"
  on "storage"."objects"
  as permissive
  for delete
  to public
using (((bucket_id = 'admin-request-docs'::text) AND (auth.uid() IS NOT NULL) AND ((storage.foldername(name))[1] = (auth.uid())::text)));
...
using (((bucket_id = 'product-documents'::text) AND (auth.uid() IS NOT NULL)));
  ...
  for delete
  to public
using (((bucket_id = 'company-logos'::text) AND (auth.uid() IS NOT NULL) AND ((storage.foldername(name))[1] = 'products'::text)));
  ...
  for delete
  to public
using (((bucket_id = 'temp-pdfs'::text) AND (auth.role() = 'authenticated'::text)));



  create policy "Users can read their own temp PDFs"
  on "storage"."objects"
  as permissive
  for select
  to public
using (((bucket_id = 'temp-pdfs'::text) AND (auth.role() = 'authenticated'::text)));
  ...
  create policy "Users can update their own admin request documents"
  on "storage"."objects"
  as permissive
  for update
  to public
...
using (((bucket_id = 'company-logos'::text) AND (auth.uid() IS NOT NULL) AND ((storage.foldername(name))[1] = 'products'::text)));
  ...
  create policy "Users can upload admin request documents"
  on "storage"."objects"
  as permissive
  for insert
  to public
with check (((bucket_id = 'admin-request-docs'::text) AND (auth.uid() IS NOT NULL) AND ((storage.foldername(name))[1] = (auth.uid())::text)));
  ...
  for insert
  to public
with check (((bucket_id = 'company-logos'::text) AND (auth.uid() IS NOT NULL) AND ((storage.foldername(name))[1] = 'products'::text)));
...
with check (((bucket_id = 'avatars'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
  ...
  for insert
  to public
with check (((bucket_id = 'admin-request-docs'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));



  create policy "Users can view their own admin request documents"
  on "storage"."objects"
  as permissive
  for select
  to public
using (((bucket_id = 'admin-request-docs'::text) AND (auth.uid() IS NOT NULL) AND ((storage.foldername(name))[1] = (auth.uid())::text)));
  ...
  for select
  to public
using (((bucket_id = 'admin-request-docs'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
