-- Fix: Use company_slug instead of company_id UUID in notification URL when NDA is validated
-- The supplier route uses company_slug from company_revision, not the UUID
...
'Creates notifications when an NDA is validated: company (both) and RFX owner/editors (in-app). Uses company_slug for supplier URL.';
