-- Fix: Remove reference to non-existent au.email column
-- The app_user table doesn't have an email column; email is in auth.users
-- We'll get the creator name from app_user only, or fallback to 'A team member'
...
'Creates in-app and email notifications to all companies related to an RFX when an announcement is posted (SECURITY DEFINER). Fixed to not reference non-existent email column.';
