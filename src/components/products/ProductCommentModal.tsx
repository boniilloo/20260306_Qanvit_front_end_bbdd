...
interface ProductCommentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (comment: string) => Promise<string | void>; // Returns product revision ID
  isSaving?: boolean;
}

export const ProductCommentModal: React.FC<ProductCommentModalProps> = ({
  isOpen,
  onClose,
  onSave,
  isSaving = false
  ...
  const [productRevisionId, setProductRevisionId] = useState<string | null>(null);
  ...
  const handleSave = async () => {
    try {
      ...
      if (revisionId && typeof revisionId === 'string') {
        setProductRevisionId(revisionId);
        setStep('embeddings');
        generateEmbeddings(undefined, revisionId); // Pass as product revision ID
      } else {
  ...
  const handleRetry = () => {
    if (productRevisionId) {
      retryGeneration(undefined, productRevisionId); // Pass as product revision ID
    }
  };
      ...
      <div className="space-y-4 py-4">
        <div className="space-y-2">
          <Label htmlFor="comment">
            Describe the product changes made (optional)
          </Label>
          <Textarea
            id="comment"
            value={comment}
            onChange={(e) => setComment(e.target.value)}
            placeholder="e.g., Updated product description, added new features, changed categories..."
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          {embeddingStatus.status === 'finished' && (
            <>
              ...
              <p className="text-sm text-green-600">Product embeddings generated successfully!</p>
            </>
          )}
          ...
          {embeddingStatus.status === 'error' && (
            <>
              ...
              <div className="text-center space-y-2">
                <p className="text-sm text-destructive">Error generating product embeddings</p>
