-- Fix get_rfx_members function to allow access for suppliers with all acceptance statuses
-- Previously it only checked for 'supplier evaluating RFX', but suppliers can also have
-- 'waiting NDA signing' and other acceptance statuses
...
comment on function public.get_rfx_members(uuid) is 'Returns all members for an RFX including owner, with email, name, surname, role, avatar_url and owner_id. Accessible by owner, members of the RFX, suppliers with active invitations (all acceptance statuses), or developers.';
